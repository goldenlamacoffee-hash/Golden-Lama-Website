...
import { getDiscardRecords, insertDiscardRecord, logHaccpAudit } from '@/lib/haccp'
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(await getDiscardRecords({ from, to }))
}
...
export async function POST(request: Request) {
  ...
  const { recordDate, product, quantity, reason, expiryDate, disposalMethod, employeeId, employeeName, notes } = body

  if (!recordDate || !product || !reason) {
    return NextResponse.json({ error: 'Chýbajú povinné polia (dátum, produkt, dôvod).' }, { status: 400 })
  }

  const record = await insertDiscardRecord({
    recordDate, product, quantity, reason,
