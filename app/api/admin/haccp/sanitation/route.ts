...
import { getSanitationRecords, insertSanitationRecord, logHaccpAudit } from '@/lib/haccp'
...
export async function GET(request: Request) {
  ...
  return NextResponse.json(await getSanitationRecords({ from, to }))
}
...
export async function POST(request: Request) {
  ...
  const { recordDate, recordTime, area, cleaningAction, productUsed, concentration, employeeId, employeeName, result, notes } = body

  if (!recordDate || !area || !result) {
    return NextResponse.json({ error: 'Chýbajú povinné polia.' }, { status: 400 })
  }

  const record = await insertSanitationRecord({
    recordDate, recordTime, area, cleaningAction, productUsed, concentration,
  ...
  await logHaccpAudit({
    entityType: 'sanitation',
    entityId: record.id,
    action: 'sanitation_recorded',
    userId: auth.user.id,
